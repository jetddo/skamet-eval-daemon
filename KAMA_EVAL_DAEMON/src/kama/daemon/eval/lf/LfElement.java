...
public class LfElement {
	...
	public String toString() {
		...
		if(LfData.State.FCST.equals(this.state)) {
			stateStr = "FCST";
		} else if(LfData.State.BECMG.equals(this.state)) {
			stateStr = "BECMG";
		} else if(LfData.State.TEMPO.equals(this.state)) {
			stateStr = "TEMPO";
		} else if(LfData.State.FM.equals(this.state)) {
			stateStr = "FM";
		}
		
		sb.append("==============================================================================\n");
		sb.append("LF 변화군 : " + stateStr + "\n");
		sb.append("LF 시각: " + datePattern.format(this.lfTm) + "\n");
	...
	public LfElement makeClone() {
		
		LfElement lfElement = new LfElement();
		
		lfElement.setLfTm(new Date(this.getLfTm().getTime()));
		lfElement.setWspd(this.getWspd());
		lfElement.setWdir(this.getWdir());
		lfElement.setMaxWspd(this.getMaxWspd());
		lfElement.setVis(this.getVis());
		lfElement.setTn(this.getTn());
		lfElement.setTx(this.getTx());
		lfElement.setState(this.getState());
		lfElement.setVrb(this.isVrb());
		lfElement.setCavok(this.isCavok());
		lfElement.setSkc(this.isSkc());
		lfElement.setNsw(this.isNsw());
		lfElement.setNsc(this.isNsc());
		lfElement.setSkyCondition(this.getSkyCondition());
		
		lfElement.setCloudAmountLayer1List(new ArrayList<String>(this.getCloudAmountLayer1List()));
		lfElement.setCloudHeightLayer1List(new ArrayList<Double>(this.getCloudHeightLayer1List()));
		lfElement.setCbCloudLayer1List(new ArrayList<Boolean>(this.getCbCloudLayer1List()));
		
		lfElement.setCloudAmountLayer2List(new ArrayList<String>(this.getCloudAmountLayer2List()));
		lfElement.setCloudHeightLayer2List(new ArrayList<Double>(this.getCloudHeightLayer2List()));
		lfElement.setCbCloudLayer2List(new ArrayList<Boolean>(this.getCbCloudLayer2List()));
	...
	public LfData.State getState() {
		return state;
	}

	public void setState(LfData.State state) {
		this.state = state;
	}
